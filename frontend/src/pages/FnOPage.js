...
const FNO_STOCKS = [
  ...
  { label:'AXISBANK',    sym:'AXISBANK'   },
  { label:'KOTAKBANK',   sym:'KOTAKBANK'  },
  { label:'MARUTI',      sym:'MARUTI'     },
  { label:'TATAMOTORS',  sym:'TATAMOTORS' },
  { label:'SUNPHARMA',   sym:'SUNPHARMA'  },
];
        ...
        <div style={{ display:'flex', gap:16, flexWrap:'wrap', alignItems:'flex-end' }}>
          ...
          <div>
            ...
            <select value={symbol} onChange={e => { setSymbol(e.target.value); setExpiry(''); }}
             ...
             {FNO_STOCKS.map(s => <option key={s.sym} value={s.sym}>{s.label}</option>)}
            </select>
          </div>
