...
_T&C Apply_`
  ...
  const s = {
    ...
    back:        { fontSize: 13, color: 'var(--text-3)', cursor: 'pointer', marginBottom: 20, display: 'inline-flex', alignItems: 'center', gap: 6, fontWeight: 600 },
    card:        { background: 'var(--surface-0)', border: '1px solid var(--line)', borderRadius: 'var(--radius-lg)', overflow: 'hidden', marginBottom: 16, boxShadow: 'var(--shadow-sm)' },
    banner:      { background: 'linear-gradient(135deg, #00b386 0%, #005a46 100%)', padding: '36px 32px', color: '#fff' },
    bannerTitle: { fontSize: 26, fontWeight: 800, marginBottom: 8, color: '#fff', letterSpacing: '-0.5px' },
    bannerSub:   { fontSize: 14, color: 'rgba(255,255,255,0.85)', lineHeight: 1.6 },
    statsRow:    { display: 'flex', borderTop: '1px solid var(--line)' },
    statBox:     { flex: 1, padding: '24px 16px', textAlign: 'center', borderRight: '1px solid var(--line)' },
    statVal:     { fontSize: 32, fontWeight: 800, fontFamily: 'var(--mono)', marginBottom: 4 },
    statLabel:   { fontSize: 11, color: 'var(--text-3)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.6px' },
    section:     { padding: '24px 28px' },
    sectionBorder: { padding: '24px 28px', borderTop: '1px solid var(--line)' },
    label:       { fontSize: 11, fontWeight: 700, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '.6px', marginBottom: 12 },
    codeWrap:    { display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' },
    code:        { padding: '14px 28px', background: 'var(--brand-dim)', border: '2px dashed var(--brand)', borderRadius: 'var(--radius-md)', fontSize: 24, fontWeight: 800, fontFamily: 'var(--mono)', letterSpacing: 4, color: 'var(--brand)' },
    copyBtn:     { padding: '15px 24px', color: '#fff', border: 'none', borderRadius: 'var(--radius-sm)', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font)', transition: 'all .15s', whiteSpace: 'nowrap' },
    shareRow:    { display: 'flex', gap: 12, flexWrap: 'wrap' },
    waBtn:       { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '14px 24px', background: '#25D366', color: '#fff', border: 'none', borderRadius: 'var(--radius-sm)', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font)', minWidth: 160, transition: 'all 0.15s' },
    linkBtn:     { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '14px 24px', background: 'var(--surface-2)', border: '1.5px solid var(--line)', borderRadius: 'var(--radius-sm)', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font)', color: 'var(--text-2)', minWidth: 120, transition: 'all 0.15s' },
    howSteps:    { display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 },
    howStep:     { display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 10, padding: '16px 12px', background: 'var(--surface-1)', borderRadius: 'var(--radius-md)', border: '1px solid var(--line)' },
    howIcon:     { width: 48, height: 48, borderRadius: '50%', background: 'var(--brand-dim)', color: 'var(--brand)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, border: '1px solid rgba(0, 179, 134, 0.2)' },
    howText:     { fontSize: 12, color: 'var(--text-2)', lineHeight: 1.45, fontWeight: 500 },
    friendRow:   { display: 'flex', alignItems: 'center', gap: 12, padding: '14px 0', borderBottom: '1px solid var(--line)' },
    ...
    badge:       (done) => ({ fontSize: 11, fontWeight: 700, padding: '4px 12px', borderRadius: 99, whiteSpace: 'nowrap', background: done ? 'var(--up-bg)' : 'var(--warn-bg)', color: done ? 'var(--up)' : '#f39c12', border: done ? '1px solid rgba(0, 179, 134, 0.2)' : '1px solid rgba(243, 156, 18, 0.2)' }),
  };
