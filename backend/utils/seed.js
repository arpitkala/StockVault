...
const seed = async () => {
  ...
  for (const s of STOCKS) {
    ...
    await Stock.create({
      ...
      open:           +(prev * (1 + (Math.random() - 0.5) * 0.005)).toFixed(2),
      high:           +(price * 1.015).toFixed(2),
      low:            +(price * 0.985).toFixed(2),
      volume:         Math.floor(Math.random() * 8e6 + 1e6),
      avgVolume:      Math.floor(Math.random() * 6e6 + 2e6),
      candlestickData: genHistory(s.basePrice, 365),
      priceHistory: genHistory(s.basePrice, 90).map(d => ({ price: d.close, timestamp: new Date(d.date) })),
  ...
  for (const f of MUTUAL_FUNDS) {
    await Fund.create({ ...f, navHistory: genHistory(f.nav * 0.6, 365).map(d => ({ nav: d.close, date: new Date(d.date) })) });
  }

  for (const idx of INDICES) {
    const v = idx.baseValue * (1 + (Math.random() - 0.5) * 0.01);
    const prev = v * (1 + (Math.random() - 0.5) * 0.01);
    await Index.create({
      ...idx,
      currentValue:  +v.toFixed(2),
      previousClose: +prev.toFixed(2),
      change:        +(v - prev).toFixed(2),
      changePercent: +((v - prev) / prev * 100).toFixed(2),
      high:          +(v * 1.01).toFixed(2),
      low:           +(v * 0.99).toFixed(2),
      candlestickData: genHistory(idx.baseValue, 365),
    });
  }

  console.log(`\n✅ Seeded: ${STOCKS.length} stocks, ${IPOS.length} IPOs, ${MUTUAL_FUNDS.length} funds, ${INDICES.length} indices`);
  process.exit(0);
};
